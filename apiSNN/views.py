...
class Expresion(generics.RetrieveUpdateDestroyAPIView):
...
class Clasificacion():
    ...
    def upload_image(request):
        ...
        if request.method == 'POST':
            ...
            dtm = datetime.today()
            var = str(int(dtm.timestamp()))
            print(var, '<', '-'*25)
            fs.save(var+upload_file.name, upload_file)
            file = open(BASE_DIR +"/"  +var + upload_file.name, 'r')
            file_path = BASE_DIR + "/" +var + upload_file.name
